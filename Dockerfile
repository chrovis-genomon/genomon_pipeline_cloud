...
RUN apt-get update && \
  ...
  pip install awscli==1.18.185 boto3==1.16.25 pyyaml==5.3.1 && \
  apt-get clean && \
  rm -rf /var/lib/apt/lists/* && \
  git clone --depth=1 -b v0.0.25 https://github.com/aokad/ecsub.git && \
  rm -rf /root/ecsub/.git

FROM python:3.9.0-slim-buster AS genomon_pipeline_cloud
